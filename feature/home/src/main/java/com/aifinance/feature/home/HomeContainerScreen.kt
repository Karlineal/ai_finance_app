...
fun HomeContainerScreen(
    ...
    var aiNavigationKey by rememberSaveable { mutableStateOf(0) }
    val openAiTabRequest by homeViewModel.openAiTabRequest.collectAsStateWithLifecycle()

    LaunchedEffect(openAiTabRequest) {
        if (openAiTabRequest) {
            selectedTopTab = HomeTopTab.AI_ASSISTANT
            aiNavigationKey++
            homeViewModel.consumeOpenAiTabRequest()
        }
    }
        ...
        Column(
            ...
            when (selectedTopTab) {
                ...
                HomeTopTab.AI_ASSISTANT -> {
                    AiAssistantScreen(
                        statisticsNavigationKey = aiNavigationKey,
                        onLoadStatisticsContext = homeViewModel::consumePendingStatisticsContext,
                    )
                }
            }
        }
